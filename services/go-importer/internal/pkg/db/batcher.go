...
// Default max time a batch waits before flushing to the DB, even if it
// hasn't reached BATCH_DEFAULT_SIZE / the per-batcher batchSize yet.
// Lower = flows show up in the UI sooner; higher = fewer, larger COPY
// calls under heavy load. Override with the BATCH_TIMEOUT_MS env var.
const BATCH_DEFAULT_TIMEOUT = time.Duration(500) * time.Millisecond;
