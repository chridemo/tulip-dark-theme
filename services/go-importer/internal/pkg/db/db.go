...
func NewDatabase(connectionString string) *Database {
	...
	// How long a batch will wait before flushing to the DB even if it
	// hasn't filled up. Defaults to BATCH_DEFAULT_TIMEOUT; override with
	// BATCH_TIMEOUT_MS (e.g. "200" for 200ms) if you need it faster/slower.
	batchTimeout := BATCH_DEFAULT_TIMEOUT
	if strMs := os.Getenv("BATCH_TIMEOUT_MS"); strMs != "" {
		if ms, err := strconv.Atoi(strMs); err == nil && ms > 0 {
			batchTimeout = time.Duration(ms) * time.Millisecond
		} else {
			log.Println("Invalid BATCH_TIMEOUT_MS, using default:", BATCH_DEFAULT_TIMEOUT)
		}
	}
	log.Println("Batch flush timeout:", batchTimeout)
