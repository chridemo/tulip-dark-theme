...
// waitForStableFile polls a file's size at pollInterval until it stops
// growing/changing (which we take to mean the writer has finished, or the
// file was already complete when it appeared), or until timeout elapses.
// This replaces a flat fixed-length sleep with a check that only waits as
// long as the file is actually still being written to, so already-complete
// files (e.g. renamed into place) get processed almost immediately.
func waitForStableFile(path string, pollInterval, timeout time.Duration) bool {
	...
	for time.Now().Before(deadline) {
		...
		if err != nil {
			// File may not exist yet (e.g. mid-rename) — keep polling.
...
func (service *AssemblerService) WatchDir(watch_dir string) {
	...
	// ProcessPcapHandle mutates shared TCP/UDP reassembly state on `service`,
	// so actual processing must stay serialized to one goroutine at a time
	// (that was implicitly true before because the fixed sleep blocked the
	// single event-loop goroutine). We still want the *waiting* for each
	// file to become stable to happen concurrently, so a fast file isn't
	// stuck behind a slow one. So: one goroutine per detected file waits
	// for stability and then hands the filename off through a channel to a
	// single dedicated consumer goroutine that does the actual processing
	// one file at a time, in the order files become ready.
	pcapReadyChan := make(chan string, 256)
	go func() {
		for name := range pcapReadyChan {
			service.HandlePcapUri(name)
		}
	}()

	// fsnotify fires multiple events per file (Create + several Write +
	// sometimes Rename). Without dedup, each one spawns its own
	// waitForStableFile goroutine, and if the file is already stable/complete
	// they all resolve almost immediately and shove the same filename into
	// pcapReadyChan repeatedly, causing the same pcap to be reopened and
	// rescanned several times in a row (wasted work, and it delays the
	// serial consumer from getting to genuinely new files).
	// pendingFiles tracks filenames that already have a stability-check
	// goroutine in flight, so extra events for the same file are ignored
	// until that goroutine finishes (enqueues or gives up).
			...
			case event, ok := <-watcher.Events:
				...
				if event.Op&(fsnotify.Rename|fsnotify.Create|fsnotify.Write) != 0 {
					...
					if strings.HasPrefix(filepath.Ext(event.Name), ".pcap") {
						...
						pendingMu.Lock()
						alreadyPending := pendingFiles[event.Name]
						if !alreadyPending {
						...
						if alreadyPending {
							// A stability check for this file is already
							// running; this event doesn't need its own.
							continue
						}

						// Wait for stability concurrently per file, but only
						// enqueue for the single serial consumer above once ready.
						go func(name string) {
							defer func() {
								pendingMu.Lock()
							...
							if waitForStableFile(name, 100*time.Millisecond, 5*time.Second) {
								pcapReadyChan <- name
							} else {
								log.Println("Gave up waiting for file to stabilize:", name)
							}
						}(event.Name)
					}
				}
