...
function CopyButton({ copyText }: { copyText?: string }) {
  ...
  return (
    <>
      {copyText && (
        <button
          className="p-2 text-sm text-cyan-400"
          onClick={copy}
          disabled={!copyText}
        >
          {statusText}
...
function highlightText(flowText: string, search_string: string, flag_string: string) {
  ...
  try {
    const searchClasses = "bg-indigo-500/40 text-indigo-100 rounded-sm";
    const flagClasses = "bg-sky-500/40 text-sky-100 rounded-sm";
...
function Flow({ full_flow, flow, flow_item_index, delta_time, id }: FlowProps) {
  ...
  return (
    <div className="text-mono" id={id}>
      <div
        className="sticky shadow-md bg-neutral-800 border-neutral-700 overflow-auto py-1 border-y"
        style={{ top: SECONDARY_NAVBAR_HEIGHT }}
      >
        <div className="flex items-center h-6">
          <div className="w-8 px-2">
            {flow.from === "s" ? (
              <ArrowCircleLeftIcon className="fill-blue-400" />
            ) : (
              <ArrowCircleRightIcon className="fill-sky-400" />
            )}
          </div>
          <div style={{ width: 200 }}>
            {formatted_time}
            <span className="text-neutral-400 pl-3">{delta_time}ms</span>
          </div>
          <button
            className="bg-neutral-700 text-neutral-100 hover:bg-neutral-600 py-1 px-2 rounded-md text-sm"
            onClick={async () => {
              window.open(
          ...
          {flowType == "Web" && flowBody && (
            <button
              className="bg-neutral-700 text-neutral-100 hover:bg-neutral-600 py-1 px-2 rounded-md text-sm ml-2"
              onClick={async () => {
                window.open(
                  "https://gchq.github.io/CyberChef/#input=" +
          ...
          <RadioGroup
            ...
            className="flex gap-2 text-neutral-200 text-sm mr-4 ml-auto"
          />
        </div>
      </div>
      <div
  ...
  return (
    <div>
      {flow.signatures?.length > 0 ? (
        <div className="bg-cyan-950 text-cyan-100 p-2">
      ...
      <div className="bg-neutral-700 text-neutral-100 p-2">
...
export function FlowView() {
  ...
  return (
    <div>
      <div
        className="sticky shadow-md top-0 bg-neutral-800 overflow-auto border-b border-b-neutral-700 flex"
      ...
      >
        {(flow?.child_id != null || flow?.parent_id != null) ? (
          <div className="flex align-middle p-2 gap-3">
            <button
              className="bg-indigo-700 text-white px-2 text-sm rounded-md disabled:opacity-50 hover:bg-indigo-600"
        ...
        <div className="flex align-middle p-2 gap-3 ml-auto">
          ...
          <select
            id="repr-select"
            value={reprId}
            className="border-2 border-neutral-600 bg-neutral-800 text-neutral-100 px-2 text-sm rounded-md"
            onChange={(e) => {
          ...
          {reprId > 0 ? <button
            className="bg-neutral-700 text-white px-2 text-sm rounded-md hover:bg-neutral-600"
