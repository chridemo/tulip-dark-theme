  ...
  return (
        <div className="flex flex-row gap-4">
          {data.map((table, tableIndex) => (
            <table key={tableIndex} className="border-collapse border border-neutral-500 table-auto">
              <thead>
                <tr>
                  <th className="border border-neutral-600 px-4">Key</th>
                  <th className="border border-neutral-600 px-4">Action</th>
                </tr>
              </thead>
              <tbody>
              {table.map((row, rowIndex) => (
                <tr key={rowIndex}>
                  {Object.entries(row).map((cell, cellIndex) => (
                    <td className="border border-neutral-700 px-4" key={cellIndex}>
                      {cell[1]}
                    </td>
                  ))}
                </tr>
...
export function Home() {
  return (
    <div className="p-4 flex flex-col gap-4 justify-center items-center h-full opacity-40">
      ...
      <h1 className="text-5xl text-neutral-300">Welcome to Tulip</h1>
      <h1 className="text-2xl text-neutral-400">Shortcut reference:</h1>
