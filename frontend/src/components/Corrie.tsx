...
export const Corrie = () => {
  ...
  const inactiveButtonClass = "bg-neutral-700 text-neutral-200 hover:bg-neutral-600 rounded-md px-2 py-1";
  const activeButtonClass = `bg-cyan-800 text-cyan-100 rounded-md px-2 py-1 ring-2 ring-cyan-500`;
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="text-sm bg-neutral-800 border-b-neutral-700 border-b shadow-md flex flex-col text-neutral-100">
...
function UnderAttackGraph(graphProps: GraphProps) {
  ...
  for (const service in ranges) {
    for (const range of ranges[service]) {
      series.push({
        // Keep every per-service bar on the same blue scale as the rest
        // of the UI instead of falling back to ApexCharts' default
        // rainbow palette.
        color: tagToColor(service),
        data: [
          {
            x: service,
