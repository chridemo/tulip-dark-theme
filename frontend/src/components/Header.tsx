...
function Diff() {
  ...
  return (
    <button
      className=" bg-indigo-900 text-indigo-200 rounded-md px-2 py-1 hover:bg-indigo-800"
      onClick={() => {
        navigateToDiff()
      }}
    >
  ...
  return (
    <>
      ...
      <Link to={`/corrie?${searchParams}`}>
        <div className="bg-cyan-900 text-cyan-200 rounded-md px-2 py-1 hover:bg-cyan-800">
          Graph view
        </div>
      </Link>
      <div className="ml-auto mr-4" style={{ display: "flex" }}>
        ...
        >
          <span className="text-neutral-300">Current: {currentTick}</span>
        </div>
      </div>
    </>
  );
