...
export function FlowList() {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="bg-neutral-800 border-b-neutral-700 border-b shadow-md flex flex-col">
        <div className="p-2 flex" style={{ height: 50 }}>
          <button
            className="flex gap-1 items-center text-sm"
            onClick={() => setShowFilters(!showFilters)}
          >
            {<FilterIcon height={20} className="text-neutral-400"></FilterIcon>}
        ...
        {showFilters && (
          <div className="border-t-neutral-700 border-t p-2">
            <div className="flex">
              <p className="text-sm font-bold text-neutral-300 pb-2">
                Intersection filter
              </p>
              <button
                className="w-24 h-5 bg-neutral-700 text-cyan-300 text-sm rounded-md ml-auto"
...
function FlowListEntry({ flow, isActive, onHeartClick }: FlowListEntryProps) {
  ...
  const duration =
    flow.duration > 10000 ? (
      <div className="text-sky-400">&gt;10s</div>
    ) : (
  ...
  return (
    ...
    >
      <div className="flex">
        ...
        >
          {isStarred ? (
            <HeartIcon className="text-sky-400" />
          ) : (
            <EmptyHeartIcon />
          )}
        </div>
        ...
        <div className="flex-1 shrink">
          <div className="flex">
            <div className="shrink-0">
              <span className="text-neutral-100 font-bold overflow-ellipsis overflow-hidden ">
                {flow.service_tag}
              </span>
              <span className="text-neutral-400">:{flow.dst_port}</span>
            </div>

            <div className="ml-2">
              <span className="text-neutral-400">{formatted_time_h_m_s}</span>
              <span className="text-neutral-500">{formatted_time_ms}</span>
            </div>
            <div className="text-neutral-400 ml-auto">{duration}</div>
          </div>
          <div className="flex gap-2 flex-wrap">
